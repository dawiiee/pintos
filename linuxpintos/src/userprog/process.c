...
process_execute (const char *file_name) 
{
  ...
  struct data_struct aux_data;
  aux_data.fn_copy = fn_copy;

  sema_init(aux_data.sema, 0);
  
  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (file_name, PRI_DEFAULT, start_process, &aux_data);
  sema_down(aux_data.sema);
  if (tid == TID_ERROR)
...
{
  struct data_struct *data = (struct data_struct*)file_name_;
  //char *file_name = file_name_->aux_data;
